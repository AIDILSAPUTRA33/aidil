...
class Tiket extends REST_Controller {
    ...
    public function index_get(){
        $id = $this->get('id');
        if($id != ''){
            $data_mhs = $this->Tiket_model->getTiket($id);
        }else{
            $data_mhs = $this->Tiket_model->getTiket();
        }
        if($data_mhs == true){
          $this->response(array(
            'status'  => 'true',
    ...
    public function tambah_post(){
        if($this->Tiket_model->tambahDataTiket($_POST) > 0) {
          $this->response(array(
            'status'  => 'true',
            'code'    => '200',
    ...
    public function ubah_post(){
      if($this->Tiket_model->ubahDataTiket($_POST) > 0) {
        $this->response(array(
          'status'  => 'true',
          'code'    => '200',
    ...
    public function hapus_get($id){
      if($this->Tiket_model->hapusDataTiket($id)) {
         $this->response(array(
          'status'  => 'true',
          'code'    => '200',
